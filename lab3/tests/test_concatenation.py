...
class TestBuchiAutomataConcatenationClass(unittest.TestCase):
    def test_concatenation(self):
        ...
        automaton_a = BuchiAutomaton(states_a, alphabet_a, transitions_a, initial_state_a, accepting_states_a)
        ...
        automaton_a1 = BuchiAutomaton(states_a1, alphabet_a1, transitions_a1, initial_state_a1, accepting_states_a1)
        ...
        self.assertEqual(concatenated_automaton.initial_state, automaton_a.initial_state)
        self.assertEqual(concatenated_automaton.accepting_states, automaton_a1.accepting_states)
        self.assertEqual(concatenated_automaton.alphabet, automaton_a.alphabet & automaton_a1.alphabet)
        ...
        for state_a, transitions_a in concatenated_automaton.transitions.items():
            ...
            for symbol, next_states_a in transitions_a.items():
                if state_b in automaton_a.accepting_states:
                    next_states_c = automaton_a1.transitions.get(state_c, {}).get(symbol, set())
                    for next_state_c in next_states_c:
                        self.assertIn((state_b, next_state_c), next_states_a)
                else:
                    next_states_b = automaton_a.transitions.get(state_b, {}).get(symbol, set())
                    next_states_c = automaton_a1.transitions.get(state_c, {}).get(symbol, set())
