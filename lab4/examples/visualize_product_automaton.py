...
def main():
    ...
    dot = graphviz.Digraph(comment='Enhanced Direct Product of Kripke Model and Büchi Automaton')
    ...
    for state in product_states:
        ...
        if is_initial and is_accepting:
            dot.node(str(state), f'{state}\n(Initial, Accepting)', shape='doubleoctagon', style='filled',
                     color='lightblue')
        elif is_initial:
            dot.node(str(state), f'{state}\n(Initial)', shape='octagon', style='filled', color='lightgrey')
        elif is_accepting:
            dot.node(str(state), f'{state}\n(Accepting)', shape='doublecircle', style='filled', color='lightgreen')
        else:
            dot.node(str(state), f'{state}')
    ...
    for state, transitions in product_transitions.items():
        for next_state in transitions:
            ...
            label = f'Kripke: {state[0]}→{next_state[0]}\nBüchi: {state[1]}→{next_state[1]}'
