...
def visualize_original_automaton_with_times(
    states,
    initial_states,
    final_states,
    transitions,
    name="Original Automaton with Times",
):
    """
    ...
    for state in states:
        ...
        if state in initial_states and state in final_states:
            dot.node(
                state_label,
                state_label,
                shape="doubleoctagon",
...
original_transitions = [
    (
        ...
        "a",
        {"x=": 1},
    ),  # From state 0, on reading 'a', stay in state 0 with x = 1
    (
        state_0,
        state_1,
        "b",
        {"y<": 1},
    ),  # From state 0, on reading 'b', move to state 1 with y < 1
    (
        state_1,
        state_0,
        "a",
        {"x<": 1},
    ),  # From state 1, on reading 'a', move to state 0 with x < 1
    (
        state_1,
        state_2,
        "b",
        {"y=": 1},
    ),  # From state 1, on reading 'b', move to state 2 with y = 1
    (
        state_2,
        state_0,
        "a",
        {"x=": 1},
    ),  # From state 2, on reading 'a', move to state 0 with x = 1
    (
        state_2,
        state_2,
        "b",
        {"y=": 1},
    ),  # From state 2, on reading 'b', stay in state 2 with y = 1
]


def main():
    # Visualizing the original automaton
    dot_original_automaton = visualize_original_automaton_with_times(
        states, initial_states, final_states, original_transitions, "Original Automaton"
    )
    dot_original_automaton.render(
        "./lab5/results/timed_automaton", format="png", cleanup=True
    )
